...
export const LogoLoop = memo(({
  ...
  const rootClassName = useMemo(() =>
    [
      ...
      .filter(Boolean)
      .join(" "), [isVertical, fadeOut, scaleOnHover, className]);
  ...
  const renderLogoItem = useCallback((item, key, itemIndex, copyIndex) => {
    if (renderItem) {
      return (
        <li className="logoloop__item" key={key} role="listitem">
          {renderItem(item, itemIndex, copyIndex)}
        </li>
      );
    }
  ...
  const logoLists = useMemo(() =>
    Array.from({ length: copyCount }, (_, copyIndex) => (
      <ul
        ...
        {logos.map((item, itemIndex) => renderLogoItem(item, `${copyIndex}-${itemIndex}`, itemIndex, copyIndex))}
      </ul>
